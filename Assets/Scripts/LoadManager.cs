    ...
    public void LoadGame()
    {
        try
        {
            if (File.Exists(Application.persistentDataPath + "\\Save.txt"))
            {
                ...
                breastplateLoot = int.Parse(data[34]);
                gauntletsLoot = int.Parse(data[35]);
                magnifyingGlassLoot = int.Parse(data[36]);
                tomeOfLuckLoot = int.Parse(data[37]);
                gemPouchLoot = int.Parse(data[38]);
                walletLoot = int.Parse(data[39]);
                alchemyKitLoot = int.Parse(data[40]);
                largeVialLoot = int.Parse(data[41]);
                highQualityIngredientsLoot = int.Parse(data[42]);
                ringOfWishesLoot = int.Parse(data[43]);
                amuletOfTimeLoot = int.Parse(data[44]);
                glovesOfMidasLoot = int.Parse(data[45]);
                manaPotionLoot = int.Parse(data[46]);
                magicFocusLoot = int.Parse(data[47]);
                tomeOfIntelegenceLoot = int.Parse(data[48]);
                summonersRobeLoot = int.Parse(data[49]);
                summonersRingLoot = int.Parse(data[50]);
                summonersStaffLoot = int.Parse(data[51]);
                tomeOfCharismaLoot = int.Parse(data[52]);
                loadedDiceLoot = int.Parse(data[53]);
                tomeOfStrengthLoot = int.Parse(data[54]);
                swiftBootsLoot = int.Parse(data[55]);
                tomeOfDexterityLoot = int.Parse(data[56]);
                poisonVialLoot = int.Parse(data[57]);
                tomeOfEnduranceLoot = int.Parse(data[58]);
                sharpeningStoneLoot = int.Parse(data[59]);
                investmentsLoot = int.Parse(data[60]);
                adventuringVoucherLoot = int.Parse(data[61]);
                dungeonMapLoot = int.Parse(data[62]);
                portalStoneLoot = int.Parse(data[63]);
                pendantOfTheDawnLoot = int.Parse(data[64]);
                year = int.Parse(data[65].Substring(0, 4));
                month = int.Parse(data[65].Substring(5, 2));
                day = int.Parse(data[65].Substring(8, 2));
                hour = int.Parse(data[65].Substring(11, 2));
                minute = int.Parse(data[65].Substring(14, 2));
                second = int.Parse(data[65].Substring(17, 2));
                chestCount = int.Parse(data[66].Substring(0, 3));
                ...
                {
                    chestList[i] = int.Parse(data[66].Substring(5 + (6 * i), 5));
                }
